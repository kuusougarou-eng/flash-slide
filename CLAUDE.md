...
## 品質基準(2026-09-10)
- 合格の定義は `docs/quality-standard.md`。視認性・レイアウト・伝達力の 3 観点で、`test/quality.test.js` が機械判定する。
- **表は主張しなければならない**: 結論に名前が出ている行だけを濃くする(全行を濃くすると強調が消える)。単位は列見出しへ。
  数値列は多数決(7 割)で右揃え。列見出しの前置き(「論点:」)は列見出しへ昇格させ、再正規化で戻さない(収束すること)。
- **分割は 3 行以上の格子だけ**、12pt でも入らないときの最後の手段。2 枚目の番号は続きから(`numberFrom`)。
  図形の格子は版面を埋めるのが正しい姿なので、fill を分割の引き金にしない。
- 実機の採点で見つけた欠陥は、まず品質ゲートに条件として足してから直す(同じ崩れを二度と通さない)。
...
## 版面の作り方(2026-09-10、実機の描画を見て決めた)
- **タイトルは 10〜18 字**、**リードは版面で 2 行に収まる長さ**(目安 60 字以内)。
- **格子は版面の下端まで使う**。余りを行に配りきる(下だけ大きく空くと置き場所を間違えたように見える)。
- **縦の揃えは行単位**。行の中に 2 行以上のセルが 1 つでもあれば、行見出しも含めてその行は全部上揃え。
  セルごとに決めると、同じ行なのに 1 行のセルだけ中央に落ちて頭がずれる。
- **余りは上 35% / 下 65% に配る**(`opticalTop`)。ど真ん中だとリード文との間が空いて本文が浮き、上詰めだと下半分が空く。
- **横長の箱は左揃え**。見出しと本文を 1 組にして縦中央に置く(中央揃えだと視線の始点が行ごとに動く)。本文を薄いグレーにしない。
- **矢羽の先端は自分の列(行)の中に収める**。隣に重ねると先端が次の矢羽に潰される。
- **列見出しの文字は一番狭い列で 1 行に収まる大きさ**まで落としてから使う(折り返すと行見出しと高さが合わない)。
- 薄い地は `#EAEAEA`(`#F2F2F2` は実機で沈んで見出しの地として認識されなかった)。
- 縦に積んだ箱フローは箱の間に薄い下向きの三角を置く(横並びのときと同じ扱い)。
