#!/usr/bin/env python3
"""ゴールデンスライドの機械チェック(PPTX)。

出どころ: carnot-tech/consulting-pptx-skill の scripts/check_deck.py(MIT）を
このプロジェクト用に移植・簡略化したもの。判定ロジックは元のまま、ブランド固有の
設定(OLD_COLORS)は空のまま、cp932環境向けにUTF-8出力を強制する行を追加している。
判断基準の正典は元リポジトリの references/slide-rules.md。

使い方:
  python scripts/golden-lint.py docs/golden/golden-v2.pptx

このプロジェクトのタイトルは「結論+根拠の数値」を1行に収める設計のため、タイトル長の
WARN は元テンプレ(18.5pt・別幅)の文字数モデルに基づく参考値。実際に2行で収まっているかは
必ず実描画(PNG)で確認すること。塗りありボックスの枠線WARNも、白地キャンバス上で白塗り
図形の輪郭を示すためのボーダーは意図的な例外として許容してよい。
