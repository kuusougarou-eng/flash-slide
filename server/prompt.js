...
複数パネルの各領域は基本テキスト。ただし 2 パネルのときに限り、片側だけを図にしてよい(図 + 意味合いの構え)。
その場合そのパネルを {"head":"図の見出し","body":{下の主図の JSON}} と書く。図の側が約 2 : 1 で広く組まれ、もう片側に読み取り・意味合いを書く。
図を横に 2 つ並べてはいけない。1 枚に図は 1 つまで。ガント・体制図・密な表は幅がいるので 1 パネルのときだけ使う。

# JSON の形式（JSON だけを返す）
1 パネル: {"panelCount":1,"title","lead"?,"footnote"?,"body":図またはcell,"note"?:短い補足文字列}
2・3 パネル: {"panelCount":2|3,"title","lead"?,"footnote"?,"panels":[{"head","text"? | "items":[文字列または子箇条書きの配列] | "body":{主図}},…]}
  2 パネルのときだけ、片側の panel を "body" に主図を入れた図のパネルにできる(1 枚に図は 1 つ)。
  例: "panels":[{"head":"粗利率の推移(%)","body":{"type":"line","unit":"%","labels":["FY23","FY24","FY25"],"series":[{"label":"当社","values":[31.2,28.0,25.1]}]}},{"head":"意味合い","items":["**値引き拡大 2.7pt**：自社の運営要因","…"]}]
  系列や量の比較が入力にあるなら、その数値は箇条書きに並べず必ず body の図にする。
...
- title: 主語と述語を持つ結論の常体文。**10〜18 字**。読み手が一目で受け取れる長さに切り詰める(修飾・条件・数字の羅列を入れない)。見出しラベルの前置きや内容と無関係な個数を入れない。
- lead: title と重ならない根拠。**版面で 2 行に収まる長さ(目安 60 字以内)**の 1〜2 文。不要なら空文字。
- **title か lead で主役を名指しする**。表なら主役の行名(または列名)、列挙なら主役の項目名を、そのままの語で含める。表示側は「結論に名前が出ている対象」だけを強調するので、名指しが無いスライドはどこも立たない。
- 表の行の並びは主張に従う。主役の行を最上段に置くか、主張の軸(成長率・金額など)で並べ替える。入力の順のまま並べない。
...
- note: 1 パネルの主図の下に置く、主図と幅を揃えた無地のテキストボックス 1 個だけ。短い補足・条件に限る。
  **note は既定で出さない。** 出してよいのは、入力にあって本文のどこにも入らない注記・出典・前提条件だけ。
  結論・要約・対応方針は lead に一度書けば足りる。lead と同じことを言い換えて主図の下に置くのは、読み手には重複でしかない。迷ったら出さない。2・3 パネルに note は付けない。
- rows / cols の自由な入れ子は出力しない。座標・色・文字サイズ・塗り・highlight は出力しない。本文の部分的な **太字** は使ってよい。
  優先する打ち手は文章で述べる。右側・先頭・最重要というだけで黒い箱やハイライトを指定しない。

# 主図の種類(1 パネルの body、または 2 パネルの片側の body に置く)
1. 文章: {"type":"cell","head"?,"text"? | "items":[…]}
2. マトリクス: {"type":"table","corner"?,"colHeaders":[…],"rows":[{"head"?,"cells":[文字列,…]},…],"numbered"?,"headShape"?,"colGroups"?,"rowGroups"?,"axes"?}
   n×m のセルを配置し、行・列名には背景色を付ける。列数 m<=2 は列名の帯を、行数 n<=2 は行名の帯を作らない。
   行・列名が区別に必要な場合は本文セルに意味を含める。情報を削除しない。
   行見出しが2階層になる(複数の行を1つの上位区分でまとめる)場合は rowGroups:[{"text","span"}] を使う。
   span はその区分に含まれる行数(合計が行数と一致するように)。上位区分が不要な平らな表では省略する。
   ...
   headShape:"chevron" は、行見出し(head)があれば行方向の矢羽、行見出しが無く列見出しだけなら列方向(左→右)の矢羽になる。工程が列方向に進む n×m の表では head を省いて colHeaders に工程名を置く。
   比較の主役になっている軸(結論に名前が出ている側)だけ headFill:"dark"(列名帯を濃く)または rowHeadFill:"dark"(行名を濃く)にできる。両方は指定しない。名前が結論に出ていない軸は指定しても既定に戻る。
...
6b. 論点ツリー: {"type":"tree","root":{"label","sub"?,"children":[同形式,…]}}
   体制図と同じ形だが、根を左に置いて右へ展開する。問いの分解、原因の分解、施策の優先度の整理に使う。
   接続は「上位の問い → その下位の論点」。数値・状態は sub に短く書く。深さ 4、葉 10 個まで。
   末端に note を付けると、右に同じ縦幅の説明列が並ぶ(任意)。根拠・数値・検証結果を 1〜2 文で書くと読み手が判断できる。
   例: {"label":"値引きの拡大","sub":"2.7pt","note":"平均値引き率 4.8% → 9.7%。上位 20 顧客に総額の 73% が集中"}
   報告関係(誰が誰に報告するか)は org、問いや原因の分解は tree を選ぶ。
...
# 入力の形ごとの読み方
- **Markdown の構成案**(「## 見出し」「- 箇条書き」「**太字**」「| 表 |」を含む): 見出しは章・パネルの候補、太字は主役の候補、
  箇条書きはそのまま items、Markdown の表はそのまま table にする。見出しが 3 つ以上並ぶなら sections か複数枚を検討する。
  構成案の指示文(「ここに図を入れる」「後で埋める」など)は本文に載せない。
- **雑然としたメモ**(順序がばらばら・口語・重複あり): まず事象と数値を拾い出し、重複を 1 つにまとめ、
  事象 × 観点 の形に組み直す。書き手の口語(「〜な感じ」「たぶん」)は落として常体にする。推測は入力に無ければ足さない。
...
- ステップ・ガント・体制図・論点ツリー・密な表は単独で版面を使う。
- 数値図(推移・量の比較・構成比)に読み取り・内訳・示唆が付くときは、図を片側に置いた 2 パネルにする。図の側の panel を {"head":…,"body":{図}} とし、もう片側に意味合い・内訳・So what を書く。数値を見ればそれで足りるなら 1 パネルでよい。
...
- **入力にある数値は 1 つ残らず載せる**。内訳を合計に丸めない(「初期費用 4.2 億円、運用費 年 6,000 万円」を「5 年総額 7.2 億円」だけにしない)。
  ...
  **入り切るかどうかは気にしなくてよい**。文字が小さくなる・行が増えるといった収まりの調整は表示側が行い、
  1 枚に入らなければ表示側が自動で次の枚へ送る。**枚数や分量を理由に事実を削るのが最も悪い**。
  入力が長い(目安 600 字以上)ときは、要約して 1 枚にまとめようとせず、素材の事実をそのまま構造化して載せる。
...
function buildMessages({ prompt, context, hint, maxSlides, variant = 0, allowSections = true }) {
  ...
  user += `\n\n【枚数】既定は 1 枚。独立した結論が 2 つあるときだけ 2 枚(slides、最大 ${maxSlides || 2} 枚)。${more}量が多いだけなら分割せず、格子や 2 階層の箇条書きに構造化して収める。ただし構造化しても入り切らない量なら、事実を削らずそのまま出す(収まりは表示側が調整する)。`;
  user += "\n\nJSON のみを出力:";
  return [
