...
### 次にやること
- 表の列見出し/行見出しの `headFill:"dark"` / `rowHeadFill:"dark"` を LLM に選ばせる
  (比較の主役になっている軸を強調。既存の `enforceBudget`——title/lead に名前が出る対象だけ許可する仕組み——に乗せる)
- n×m のステップ/アプローチ表で、列見出しを矢羽根(chevron)にできるようにする(今は行見出しのみ対応)
- ラベル/値の 2 列表現(1 行だけ 2 シェイプになる)の扱いを決める(1 シェイプに畳む / 残す / ネイティブ表にする)
- 疎なスライドの余白の埋め方(間隔で埋めなくなった分、本文サイズの上限 20pt だけでは足りないケースがある)
