...
assert.strictEqual(listPrim.text.split("\n").filter((l) => /^　　\S/.test(l)).length, 2, "child rows are indented inside the shape");
assert.ok(listPrim.bullets && !/[•–]/.test(listPrim.text), "native bullets, no pseudo markers in the text");
